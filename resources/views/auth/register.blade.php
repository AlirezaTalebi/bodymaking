    ...
    <div class="w-full max-w-md mx-auto p-6 bg-gradient-to-br from-zinc-900 to-gray-800 rounded-lg text-white" >
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4"  x-cloak>
                ...
                <div x-data="{ genderDropDown: false, selected: '{{ old('gender') ? __("general." . old('gender')) : __("general.select_gender") }}'}"
                     class="relative" >
                    <!-- Trigger -->
                    <button @click="genderDropDown = !genderDropDown" type="button"
                        ...
                        <i :class="genderDropDown ? 'fa-solid fa-chevron-up' : 'fa-solid fa-chevron-down'"></i>
                    </button>

                    <!-- Options -->
                    <div x-show="genderDropDown" x-transition @click.outside="genderDropDown = false"
                        ...
                        <ul class="py-1 text-sm text-white">
                            @foreach (User::getGender() as $gender)
                                <li>
                                    <button type="button"
                                            @click="selected = '{{ __("general.$gender") }}'; genderDropDown = false; $refs.gender.value = '{{ $gender }}'"
